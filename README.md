...
방문자는 **로그인하지 않은 외부 작성자**라, 만들어진 문서는 담당자의 「받은 문서함」에 뜨지 않는다.

- **문서 목록**: 이폼사인 콘솔 > 문서함 > **완료 문서**. 시작 단계에서 바로 완료로 넘어가므로 여기 쌓인다.
- 목록 컬럼에 방문일시·성명·소속·방문 목적·담당자가 나오도록 `display_settings` 를 잡아 두었다
  (연락처·동의·서명은 개인정보라 목록에서 숨김 — 문서를 열면 보인다).
- **API**: `documents.list` 는 `type` 파라미터로 대상을 고른다. 외부 작성자가 만든 문서는
  **`type: "02"`(회사 전체 문서) 또는 `type: "04"`(템플릿별 문서)** 로 조회한다.
  🔴 `type: "01"`(내 문서함) 으로는 **안 나온다** — 작성자가 멤버가 아니기 때문이다.
  `tools/list-docs.mjs` 가 `type:"04"` 로 템플릿별 문서를 뽑는다.
- **메일 알림**: 완료 시점 알림(`complete_document` / `complete_outsider_document`)이 켜져 있고 PDF 가 첨부된다.
  방문자는 메일 주소를 남기지 않으므로 실제 수신자는 템플릿 소유자다.
